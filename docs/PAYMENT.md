...
star-X has a built-in payment system that enables user self-service top-up without deploying a separate payment service.
...
>
> - **Choosing a payment provider**: select a provider based on your region, settlement currency, qualification requirements, fees, and terms of service. Always rely on the provider's current official documentation.
>
...
If you previously used an external payment system, you can migrate to the built-in payment system:

### Key Differences

| Aspect | External payment service | Built-in Payment |
|--------|-----------|-----------------|
| Deployment | Separate service (Next.js + PostgreSQL) | Built into star-X, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in star-X admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Not yet (planned) |
| Order Management | Separate admin interface | Integrated in star-X admin dashboard |

### Migration Steps

1. Enable payment in star-X admin dashboard and configure providers (use the same payment credentials)
2. Update webhook callback URLs to star-X's callback endpoints
3. Verify that new orders are processed correctly via built-in payment
4. Decommission the external payment service after verifying historical orders

> **Note**: Historical order data from an external payment service will not be automatically migrated. Keep the original service running for a while to access historical records.
