<template>
    ...
    <header class="site-header">
      <nav class="nav-shell" aria-label="主导航">
        <router-link to="/" class="brand" aria-label="star-X 首页">
          <img :src="siteLogo" alt="star-X" class="brand-logo" />
        </router-link>
        <div class="nav-links">
    ...
    <main id="top">
      <section class="hero section-shell">
        <div class="hero-copy">
          ...
          <h2 class="hero-product-title" aria-label="star-X API算力中转站"><strong>API</strong><span>算力中转站</span></h2>
  ...
  <span>"usage"</span>: { <span>"input_tokens"</span>: 11, <span>"output_tokens"</span>: 18 }
    ...
    <footer class="site-footer section-shell"><div class="brand footer-brand"><img :src="siteLogo" alt="star-X" class="brand-logo" /></div><span>强大的 AI API 管理平台。</span><small>© {{ currentYear }} star-X. 保留所有权利。</small></footer>
  </div>
</template>
...
const siteLogo = computed(() => sanitizeUrl(appStore.cachedPublicSettings?.site_logo || appStore.siteLogo || '/starx-logo-transparent.png', { allowRelative: true, allowDataUrl: true }))
...
@media (max-width:900px){.nav-links{display:none}.hero{grid-template-columns:1fr;gap:45px;padding-top:54px}.hero-copy{text-align:center}.hero-wordmark,.hero-product-title{justify-content:center;margin-inline:auto}.trust-list{justify-content:center}.api-panel{max-width:650px;margin:auto}.feature-grid{grid-template-columns:1fr}.metrics{grid-template-columns:1fr 1fr;gap:22px}.metric:nth-child(2){border-right:0}.capability-board{grid-template-columns:1fr}.capability,.capability:nth-child(odd){border-right:0;border-top:1px solid var(--line)}.capability:first-child{border-top:0}.benefit-row{grid-template-columns:1fr 1fr;gap:18px}.steps{gap:35px}.cta{flex-direction:column;padding:31px 24px}.site-footer{flex-wrap:wrap}.site-footer small{margin-left:0;width:100%}}
