      ...
      <template v-if="isAdmin">
        ...
        <div v-if="!authStore.isSimpleMode" class="sidebar-section">
          ...
          <router-link
            ...
            :key="item.path"
            :to="item.path"
            :target="item.path === '/recharge' ? '_blank' : undefined"
            :rel="item.path === '/recharge' ? 'noopener noreferrer' : undefined"
            class="sidebar-link mb-1"
            :class="{ 'sidebar-link-active': isActive(item.path), 'sidebar-link-collapsed': sidebarCollapsed }"
            :title="sidebarCollapsed ? item.label : undefined"
...
//
// Star-X 用户端采用外部店铺购买 + 站内兑换码到账的闭环。
// 原生渠道监控、订阅、支付和订单入口仍保留在代码中供管理端使用，但不在用户菜单展示。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/recharge', label: t('nav.balanceRecharge'), icon: RechargeSubscriptionIcon },
